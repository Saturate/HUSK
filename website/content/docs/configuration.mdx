...
| `compression.mode` | `HUSK_COMPRESSION_MODE` | `client` | `client` (disabled) or `server` (server-side) |
...
Compression triggers when `mode = "server"`: batch threshold reached, session ends, or no observations for `interval_minutes`.
