...
When `max_tokens` is set, YAMS fetches up to 50 results internally and returns as many as fit within the token budget. This lets the AI client say "give me as much context as you can in 2000 tokens" without having to guess a limit.
...
When storing, YAMS embeds the content and searches for existing memories from the same user. If the closest match scores above the dedup threshold (default 0.92), the existing memory is returned with `duplicate: true`.
